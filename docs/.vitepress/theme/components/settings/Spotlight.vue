...
<template>
  <div space-y-2 role="radiogroup">
    <SpotlightHoverBlock v-if="mounted && spotlightToggledOn && !disabled" :enabled="spotlightToggledOn && !disabled" />
    <div ref="menuTitleElementRef" relative flex items-center>
      <MenuTitle title="Spotlight" aria-label="Spotlight" :disabled="disabled" flex="1" pr-4>
        <template #icon>
          <span i-icon-park-outline:click mr-1 aria-hidden="true" />
        </template>
      </MenuTitle>
      <MenuHelp v-if="!options.spotlight?.disableHelp" v-model:is-popped-up="isMenuHelpPoppedUp"
    ...
    <NuInputHighlight :active="isMenuHelpPoppedUp" class="rounded-md">
      <NuInputHorizontalRadioGroup v-model="spotlightToggledOn"
